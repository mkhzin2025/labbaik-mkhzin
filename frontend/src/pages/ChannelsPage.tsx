    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-labbaik-surface p-8 rounded-[2.5rem] border border-purple-100/60 dark:border-white/5 shadow-2xl">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl font-black text-neutral-900 dark:text-white">إدارة القنوات المتصلة</h1>
            <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1 font-medium text-right">اربط حسابات التواصل الاجتماعي لتبدأ إدارة المحادثات بذكاء.</p>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={platform.id} className={`p-8 rounded-[2.5rem] border transition-all hover:scale-[1.02] ${platform.color} flex flex-col justify-between gap-6 group`}>
              <div className="flex justify-between items-start">
                ...
                {connected ? (
                  <span className="flex items-center gap-2 px-4 py-2 bg-green-500/10 border border-green-500/20 text-green-600 dark:text-green-400 text-[10px] font-black rounded-full uppercase tracking-widest">
                    <CheckCircle2 size={12} /> متصل الآن
                  </span>
                ) : (
                  <span className="flex items-center gap-2 px-4 py-2 bg-white/40 dark:bg-white/5 border border-black/5 dark:border-white/10 text-neutral-600 dark:text-neutral-400 text-[10px] font-black rounded-full uppercase tracking-widest">
                    غير مرتبط
                  </span>
                )}
              </div>
              ...
              <div>
                <h3 className="text-xl font-black text-neutral-900 dark:text-white mb-2">{platform.name}</h3>
                <p className="text-neutral-600 dark:text-neutral-400 text-sm font-medium leading-relaxed">{platform.desc}</p>
              </div>
              ...
              <div className="flex items-center gap-4">
                {connected ? (
                  <>
                    ...
                    <button onClick={() => platform.id === 'whatsapp' && navigate('/dashboard/settings?tab=meta')} className="p-4 bg-white/40 dark:bg-white/5 text-neutral-600 dark:text-neutral-400 rounded-2xl border border-black/5 dark:border-white/5 hover:text-neutral-900 dark:hover:text-white transition-all"><ExternalLink size={18} /></button>
                  </>
                ) : (
                  <Button 
          ...
          <div className="bg-labbaik-surface w-full max-w-lg rounded-[2.5rem] border border-purple-100/60 dark:border-white/10 shadow-3xl overflow-hidden relative">
            ...
            <div className="p-10 space-y-8">
              <div className="text-center">
                ...
                <h2 className="text-2xl font-black text-neutral-900 dark:text-white">إعدادات الربط</h2>
                <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-2 font-medium">أدخل بيانات Cloud API الخاصة بـ Meta</p>
              </div>

              <div className="space-y-4">
                {selectedType === 'whatsapp' && (
                  <div className="bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-5 space-y-3 text-right">
                    <p className="text-[10px] font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest">Meta Webhook</p>
                    <div className="space-y-2">
                      <div className="text-[11px] text-neutral-500 dark:text-neutral-400 font-bold">Callback URL</div>
                      <code className="block bg-black/5 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-xl p-3 text-[11px] text-labbaik-blue break-all ltr text-left">{webhookUrl}</code>
                    </div>
                    <div className="space-y-2">
                      <div className="text-[11px] text-neutral-500 dark:text-neutral-400 font-bold">Verify Token</div>
                      <code className="block bg-black/5 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-xl p-3 text-[11px] text-labbaik-blue break-all ltr text-left">{creds.verifyToken}</code>
                    </div>
                  </div>
                )}
                <div className="space-y-2">
                  <label className="text-xs font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest me-2">Phone Number ID</label>
                  <input 
                    type="text" 
                    value={creds.phoneNumberId}
                    onChange={(e) => setCredentials({...creds, phoneNumberId: e.target.value})}
                    placeholder="1234567890..."
                    className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-6 text-sm text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-labbaik-blue/50 transition-all font-bold"
                  />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest me-2">Display Phone</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest me-2">WABA ID</label>
                    <input 
                      type="text" 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-neutral-500 dark:text-neutral-400 uppercase tracking-widest me-2">Permanent Access Token</label>
                  <textarea 
                    value={creds.accessToken}
                    onChange={(e) => setCredentials({...creds, accessToken: e.target.value})}
                    placeholder="EAAP..."
                    className="w-full h-32 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-6 text-sm text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-labbaik-blue/50 transition-all font-bold resize-none"
                  />
                </div>
              </div>
